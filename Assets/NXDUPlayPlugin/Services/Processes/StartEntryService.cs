...
{
    private const int LAUNCH_TIMEOUT = 30000;

    private HashSet<string> _cachedExecutables;

    public GameActionResult StartEntry(UPlayPlugin plugin, LibraryEntry entry, CancellationToken cancellationToken)
    {
        ...
        Process.Start(new ProcessStartInfo
        {
            ...
            Arguments = $"\"uplay://launch/{entry.EntryId}\"",
    ...
    private async UniTask MonitorGameDirectoryAsync(UPlayPlugin plugin, LibraryEntry entry, CancellationToken cancellationToken)
    {
    ...
    private bool HasProcessesInDirectory(string gameDirectory)
    {
        ...
        {
            Debug.Log($"Checking if {exeName} is running");
            try
            {
    ...
    private HashSet<string> GetKnownExecutables(string gameDirectory)
    {
        ...
        {
            Debug.LogError($"Error scanning directory: {ex.Message}");
        }
